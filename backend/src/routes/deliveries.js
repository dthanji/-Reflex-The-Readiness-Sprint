...
router.get('/', requireAuth, async (req, res) => {
  try {
    ...
    if (req.user.role === 'dispatcher') {
      query = `${base} ORDER BY s.created_at DESC`; params = [];
    } else if (req.user.role === 'retailer') {
      query = `${base} WHERE s.retailer_id = $1 ORDER BY s.created_at DESC`; params = [req.user.id];
    } else {
      query = `${base} WHERE s.rider_id = $1 ORDER BY s.created_at DESC`; params = [req.user.id];
    }
...
// Retailers may change the assigned rider only after the current rider
// reports that they cannot complete the delivery (FAILED). This preserves
// an auditable handoff instead of silently replacing an active rider.
router.put('/:id/reassign', requireAuth, requireRole('retailer'), async (req, res) => {
  ...
  try {
    ...
    const assignment = await client.query(
      `INSERT INTO assignments (delivery_request_id, rider_id, assigned_by) VALUES ($1, $2, $3) RETURNING *`,
      [requestId, rider_id, req.user.id]
    );
    const event = await client.query(
      `INSERT INTO status_events (delivery_request_id, status, actor_id, metadata) VALUES ($1, 'ASSIGNED', $2, $3) RETURNING *`,
      [requestId, req.user.id, JSON.stringify({ rider_id, reassigned: true, reason: 'Previous rider reported a delivery issue' })]
    );
    await client.query('COMMIT');
...
router.get('/:id/history', requireAuth, async (req, res) => {
  try {
    const { rows } = await pool.query(
      `SELECT se.*, u.name AS actor_name, u.role AS actor_role FROM status_events se JOIN users u ON u.id = se.actor_id WHERE se.delivery_request_id = $1 ORDER BY se.created_at ASC`,
      [req.params.id]
    );
