...
function generateDeliveryCode() {
  return `RFX-${crypto.randomBytes(4).toString('hex').toUpperCase()}`;
}

// Retailer logs a new delivery request. The delivery code is generated by
// the server and is globally unique, so riders cannot invent or reuse a code.
    ...
    for (let attempt = 0; attempt < 5; attempt += 1) {
      ...
      try {
        const result = await client.query(
          `INSERT INTO delivery_requests (retailer_id, customer_name, customer_phone, address, item_description, delivery_code)
           VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
          [req.user.id, customer_name, customer_phone, address, item_description, deliveryCode]
        );
        request = result.rows[0];
        break;
      } catch (err) {
        if (err.code !== '23505' || !String(err.constraint || '').includes('delivery')) throw err;
      }
    }
