...
function copyDeliveryCode(code) { navigator.clipboard?.writeText(String(code)).then(() => { alert('Delivery code copied. Share it with the customer.'); }).catch(() => alert(`Delivery code: ${code}`)); }
...
function renderModal(){if(!state.modal)return'';if(state.modal.type==='assign'){const options=state.riders.map(r=>`<div class=\"rider-option\" onclick=\"assignRider(${Number(state.modal.requestId)},${Number(r.id)})\"><span>${escapeHtml(r.name)}</span><span class=\"mono\" style=\"color:#8A8A7E\">${escapeHtml(r.phone)}</span></div>`).join('');return `<div class=\"modal-backdrop\" onclick=\"if(event.target===this)closeModal()\"><div class=\"modal-sheet\"><h3>Assign to a rider</h3>${options||'<p style=\"color:#8A8A7E;font-size:13px\">No riders registered yet.</p>'}<button class=\"btn secondary\" style=\"margin-top:8px\" onclick=\"closeModal()\">Cancel</button></div></div>`;}return `<div class=\"modal-backdrop\" onclick=\"if(event.target===this)closeModal()\"><div class=\"modal-sheet\"><h3>Confirm delivery</h3><div id=\"qr-reader\" style=\"width:100%;min-height:220px\"></div><div class=\"field\"><label>Customer code</label><input id=\"manual-qr-input\" placeholder=\"RFX-XXXXXXXX\" autocomplete=\"off\"></div><button class=\"btn amber\" type=\"button\" onclick=\"manualConfirm(${Number(state.modal.requestId)})\">Confirm</button><button class=\"btn secondary\" type=\"button\" onclick=\"closeModal()\">Cancel</button></div></div>`;}
function render(){if(!state.user){app.innerHTML=renderAuth();return;}app.innerHTML=`<header class=\"topbar\"><div><strong>REFLEX</strong><span>${escapeHtml(state.user.role)}</span></div><button class=\"btn secondary\" onclick=\"logout()\">Sign out</button></header><main class=\"shell\">${state.user.role==='retailer'?renderRetailerView():state.user.role==='dispatcher'?renderDispatcherView():renderRiderView()}</main>${renderModal()}`;}
function renderAuth(){return `<div class="auth-shell"><div class="auth-card"><div class="brand">REFLEX</div><h1>${state.authMode==='login'?'Welcome back':'Create account'}</h1><p class="auth-tagline">${state.authMode==='login'?'Sign in to view your deliveries and track status in real time.':'Register to log requests, dispatch riders, or manage your delivery route.'}</p><form onsubmit="handleAuthSubmit(event)">${state.authMode==='register'?'<div class="field"><label>Name</label><input name="name" required></div>':''}<div class="field"><label>Phone</label><input name="phone" required></div><div class="field"><label>Password</label><input type="password" name="password" required></div>${state.authMode==='register'?'<div class="field"><label>Role</label><select name="role"><option value="retailer">Retailer</option><option value="dispatcher">Dispatcher</option><option value="rider">Rider</option></select></div>':''}${state.authError?`<div class="error">${escapeHtml(state.authError)}</div>`:''}<button class="btn amber" type="submit">${state.authMode==='login'?'Sign in':'Create account'}</button></form><button class="link-btn" onclick="state.authMode=state.authMode==='login'?'register':'login';state.authError=null">${state.authMode==='login'?'Create an account':'Back to sign in'}</button></div></div>`;}
