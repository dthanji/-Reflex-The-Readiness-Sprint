...
(function () {
  ...
  function isDeliveredTicket(ticket) {
    return Boolean(ticket.querySelector('.ticket-status.status-DELIVERED')) ||
      ticket.querySelector('.ticket-status')?.textContent.trim() === 'DELIVERED';
  }
  ...
  function decorateRatingTickets() {
    ...
    document.querySelectorAll('.ticket').forEach(ticket => {
      ...
      panel.innerHTML = `<div class="reflex-rate-title">${label}</div><div class="reflex-rate-buttons">${[1,2,3,4,5].map(n => `<button type="button" aria-label="${n} star${n > 1 ? 's' : ''}" onclick="window.submitRiderRating(${deliveryId}, ${n}, this)">★</button>`).join('')}</div><div class="reflex-rate-note">${note}</div>`;
  ...
  window.submitRiderRating = async function (deliveryId, rating, button) {
    ...
    try {
      const comment = window.prompt('Optional comment about the rider/delivery (leave blank to skip):', '') || '';
      ...
      if (session().user?.role === 'rider') await loadRiderSummary();
    } catch (e) {
      panel.dataset.submitting = '0';
      if (/already been rated/.test(e.message)) panel.innerHTML = `<div class="reflex-rate-success">Rating already submitted<br><small>You have already rated this delivered order.</small></div>`;
  ...
  function observe() {
    ...
    refresh();
    // app.js renders tickets asynchronously; retry briefly so the rating control
    // is present even when the ticket list is replaced after login or a status update.
    let attempts = 0;
    const retry = setInterval(() => {
      attempts += 1;
      decorateRatingTickets();
